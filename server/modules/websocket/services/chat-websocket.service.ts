 ...
 *
 * A run in progress is the ordinary case, and a process still held open after
 * its run's `complete` (background work pending) accepts an injected turn too
 * — pushing into it is exactly what keeps the single-writer lock from making
 * the author wait out the background work. Only plain text injects: stdin
 * carries text, so a queued message with attachments would lose its files
 * silently, and losing data is not an acceptable form of expedience.
 *
